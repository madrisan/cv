...
\item{\bdot}
    ...
    (HTML5, Bootstrap, Java-Script, TypeScript, jQuery, AngularJS,
    Ionic, NodeJS, Express, MongoDB, $\dots$);
    %, and a bunch of other tools in the background like
    % Gulp, Bower, nmp, less, $\dots$
\item{\bdot}
    Cloud Computing Technologies and Microservices Architectures;
\item{\bdot}
    (Big) Data Analysis and Technologies (R, Spark, Hadoop, ...), Machine Learning;
\item{\bdot}
    Python programming, frameworks and related technologies;
\item{\bdot}
...
\smallskip\nobreak\noindent
See below the list of the 40$+$ MOOCs I've completed so far.
\goodbreak
...
\item{\bdot} Cloud services based on the AWS Technologies.
...
\item{\bdot} Validate, improve and build High Availability solutions,
   ...
   technologies;
\item{\bdot} Develop an open source framework (back--end in Python, front--end in
...
\item{\bdot} Linux RHEL/CentOS/SLES, Red~Hat Cluster Suite, Docker;
\item{\bdot} Postfix mail server, BIND DNS server, Proxy Squid, NFS, SAN, awstats;
\item{\bdot} Amazon Web Services (AWS);
\item{\bdot} Salt~Stack, Travis~CI, Jenkins~CI, Git/GitLab, JSON, Python,
             HTML5, Bootstrap, CSS3/Less, AngularJS.
...
\certauthority{Santa Fe Institue}{Complexity Explorer}
   \certitem{Introduction to Dynamical Systems and Chaos}
\smallskip
...
\certauthority{University of Michigan}{Coursera}
   ...
   \certitem{Advanced Styling with Responsive Design}
...
\certauthority{Pluralsight}{Code School}
   \certitem{Shaping up with Angular.js}
\certauthority{Microsoft}{edX}
   \certitem{Introduction to JQuery}
   \certitem{Introduction to TypeScript}
\certauthority{MongoDB}{MongoDB University}
   \certitem{M101JS: MongoDB for Node.js Developers}
\certauthority{MongoDB}{edX}
   \certitem{M101x: Introduction to MongoDB using the MEAN Stack}
