...
\rightline{\bf%
Experienced Linux and OpenSource Hacker}
\rightline{\bf%
(DevOps, Cloud Computing, Development, Monitoring)}
\rightline{\bf%
Web, Data Science, Big~Data, and Machine Learning passionate.}
...
\personaldata{%
  ...
  %\personaldataline{Personal website}{\rm https:/\negthinspace/sites.google.com/site/davidemadrisan}
   \personaldataline{Personal websites}{\tt https://madrisan.wordpress.com\hbox to 7.5mm{\hss${}^{(new)}$}}
...
Embedded System and Full~Stack Web Development,
...
\noindent I'm currently busy on learning Angular2/Node.js and
Web technologies in general,
% and improving my knowledge of Scala language
but my interests in technology and maths fields cover
the following areas:

\item{\bdot}
    Maths, currently digging around Dynamic Systems and Chaos Theory;
\item{\bdot}
    Python, JavaScript/TypeScript programming;
\item{\bdot}
    Full Stack Web Development (HTML5, Bootstrap, AngularJS, Node.js, MongoDB);
\item{\bdot}
    Cloud Computing Technologies, Microservices Architectures, Docker containerization;
\item{\bdot}
    (Big) Data Analysis and Technologies (R, Spark, Hadoop, ...), Machine Learning;
\item{\bdot}
    Functional and concurrent programming (Scala and AKKA);
\item{\bdot}
    Distributed Systems and GPGPU Programming
    (CUDA C\plusplus{} programming in particular);
\item{\bdot}
    Linux, Linux embedded and DevOps Technologies.
...
\dateitem{April~2013}{current}{\OS2--Open Source Solutions (Auto Entrepreneur)}

\noindent In parallel of my current job, I have decided to develop an additional professional
activity as Auto Entrepreneur, mainly focused on Open Source and Linux--based (new) technologies:

\item{\bdot} Web development (JavaScript/JQuery, MEAN Stack);
...
\item{\bdot} Develop a service for building and packaging the infrastructure software
...
\item{\bdot} Integrate the customer's software applications on their production
   systems.

\subsection{Technologies}

\item{\bdot} Linux RHEL/CentOS, RedHat Cluster Suite, Spacewalk, Docker containers, AWS services;
\item{\bdot} Postfix mail server, BIND DNS server, Proxy Squid, NFS, SAN;
...
%with a data archiving on Amazon S3 and Glacier.
%
%\subsection{Technologies}
%
...
\section{CERTIFICATIONS \& ONLINE SPECIALIZATIONS}
...
\certauthority{Universit\'e de Strasbourg}{FUN}
   \certitembullet{Stochastic Evolutionary Optimization}
\certauthority{Stanford University}{Coursera}
   \certitembullet{Machine Learning}
\certauthority{Microsoft}{edX}
   \certitembullet{Introduction to R}
\certauthority{Hardvard University}{edX}
   \certitembullet{Statistics and R for Life Sciences}
\smallskip

\certgroup{BUSINESS}

\certauthority{Ludwig--Maximilians Universit\"at M\"unchen}{Coursera}
   \certitembullet{Competitive Strategy}
\smallskip
...
\certauthority{AWS Training}{Amazon Web Services}
   \certitembullet{AWS Technical Essentials (version August 2016)}
\smallskip
...
\certauthority{University of Illinois at Urbana-Champaign}{Coursera}
   \certitembullet{Heterogeneous Parallel Programming}
...
\certauthority{Universidad Carlos III de Madrid}{edX}
   \certitembullet{Introduction to Programming with Java -- part~1}
...
\certauthority{University of Michigan}{Coursera}
   \certitembullet{Using Python to Access Web Data}
\smallskip

\certgroup{WEB PROGRAMMING}

\certauthority{W3C}{edX}
   \certitembullet{Learn HTML5 from W3C}
...
\certauthority{Pluralsight}{Code School}
   \certitembullet{Shaping up with Angular.js}
\certauthority{Microsoft}{edX}
